...
# === NEW: สำหรับ Volume Spike & Strong SHORT Detection ===
last_spike_check = datetime.min
SPIKE_CHECK_INTERVAL = timedelta(minutes=5)  # ตรวจ Volume Spike ทุก 5 นาที

last_short_signal_check = datetime.min
SHORT_SIGNAL_CHECK_INTERVAL = timedelta(minutes=7)  # ตรวจ Strong SHORT ทุก 7 นาที
# เก็บ symbol ที่แจ้งคำแนะนำแล้ว (ไม่แจ้งซ้ำ)


# === AUTO ENTER CONTROL FLAGS ===
auto_spike_enabled = True      # เปิด Volume Spike Auto LONG โดย default
auto_short_signal_enabled = True  # เปิด Strong Short Auto SHORT โดย default
...
#MAX_LEVERAGE = 20                  # ลดจาก 20 → 10x เพื่อความปลอดภัย (ลดความเสี่ยงล้างพอร์ต)
#RISK_PER_TRADE_PERCENT = 0.025       # เพิ่มจาก 0.025 → 0.5% ของพอร์ตต่อไม้ (เหมาะกับทุนน้อย)
#MAX_OPEN_POSITIONS = 10             # ลดจาก 50 → 3 ไม้พร้อมกัน (ป้องกัน over-exposure)
#SIGNAL_THRESHOLD_LONG = 6          # เพิ่มจาก 6 → 7 (สัญญาณต้องแน่นกว่าเดิม)
#SIGNAL_THRESHOLD_SHORT = 7         # เพิ่มจาก 7 → 8 (เข้มงวดกว่า LONG เพราะตลาดลงเร็ว)
#ADX_THRESHOLD = 25                 # เพิ่มจาก 25 → 30 (เทรนด์ต้องแข็งแรงจริงๆ)
#SCAN_BATCH_SIZE = 60               # ลดจาก 60 → 30 (ลดการสแกนหนักเกินไป)
#MIN_NOTIONAL_USDT = 5              # คงไว้ (ขั้นต่ำ Binance Futures)
#MIN_BALANCE_TO_TRADE = 5.0         # ลดจาก 100 → 4 (ให้บอทเริ่มเทรดได้แม้ทุนเหลือน้อย)

# ปรับการเข้าให้รอโซนดีจริง ๆ ตาม Fibonacci
#ENTRY_PULLBACK_PERCENT = 18.8      # เพิ่มจาก 10.8 → 12.0% รอ pullback ลึกขึ้นเพื่อเข้าในจุดที่ดีกว่า (ตอนนี้ dynamic ด้วย ML)
#LIMIT_ORDER_TIMEOUT_HOURS = 2      # เพิ่มจาก 2 → 4 ชม. ให้เวลา Limit รอจับมากขึ้น
...
# ==========================================================================
#                        OPTIMIZED CONFIG FOR $50 CAPITAL
# ==========================================================================

MAX_LEVERAGE = 15                    # ลดจาก 20 → 15x (ปลอดภัยกว่า ทุนน้อยไม่ควร over-leverage)
RISK_PER_TRADE_PERCENT = 0.02        # ลดจาก 0.025 → 2% ต่อไม้ (ก่อนหน้าเสี่ยงเกินไปสำหรับ $50)
MAX_OPEN_POSITIONS = 5               # ลดจาก 10 → 5 ไม้พร้อมกัน (ทุน $50 ไม่ควรเปิดเกิน 5)
SIGNAL_THRESHOLD_LONG = 7            # เข้มงวดขึ้น: ต้องได้คะแนน 7/10 ขึ้นไปถึง LONG
SIGNAL_THRESHOLD_SHORT = 8           # เข้มงวดมากขึ้นสำหรับ SHORT (ตลาดลงเร็ว + ทุนน้อย = อันตราย)
ADX_THRESHOLD = 28                   # เพิ่มจาก 25 → 28 (ต้องเทรนด์แข็งแรงจริง ๆ)
SCAN_BATCH_SIZE = 40                 # ลดจาก 60 → 40 (ลดการสแกนหนักเกินไป)
MIN_NOTIONAL_USDT = 5                # คงไว้ (ขั้นต่ำของ Binance Futures)
MIN_BALANCE_TO_TRADE = 20.0          # ลดจาก 5 → 20 (ถ้าทุนต่ำกว่า 20 ไม่เทรดเลย เพื่อความอยู่รอด)

# ปรับ Pullback ให้รอของถูกมากขึ้น (ทุนน้อยต้องเข้าในจุดดีจริง ๆ)
ENTRY_PULLBACK_PERCENT = 13.8        # คงไว้ แต่ตอนนี้เป็น dynamic จาก AI (0-30%)

LIMIT_ORDER_TIMEOUT_HOURS = 3        # เพิ่มจาก 2 → 3 ชม. ให้เวลา Limit รอจับมากขึ้น

# Risk Management ที่เข้มงวดขึ้น
ATR_SL_MULTIPLIER = 2.2              # แน่นขึ้นจาก 2.5 → 2.2 (ตัดขาดทุนเร็ว)
ATR_TP_MULTIPLIER = 6.0              # คงไว้ → RR 1:2.7 (ยังดีอยู่)
...
# ==========================================================================
#                   NATIVE INDICATORS (เวอร์ชันแก้ไขแล้ว - ใช้ตัวนี้ตัวเดียว)
# ==========================================================================
def calculate_indicators(kline_data):
    try:
        ...
        # === PANIC SELL GUARD: straight_down detection ===
        df['straight_down'] = 0  # สร้าง column ก่อน
...
class TitanBrain:
    ...
    # เพิ่มใหม่: Predict dynamic pullback % (0-50%)
    def get_pred_pullback(self, f):
        conf = self.get_ai_confidence(f)
        return (conf / 100) * 30  # 0-50% based on confidence
    ...
    for sym in symbols:
        ...
        if best_tf and sym not in active_symbols:  # ไม่เข้าใหม่ถ้ามี position แล้ว
            ...
            try:
                ...
                report = (
                    f"🚀 *AUTO ENTERED LONG (Volume Spike)*\n"
    ...
    for sym in symbols:
        ...
        for tf in tfs:
            try:
                ...
                if short_conditions >= 6 and vol_ratio > 2.5 and strength > max_strength and sym not in active_symbols:
                    max_strength = strength
                    best_tf = tf
                    best_atr = curr['atr']
                    best_price = curr['c']
                    best_signal = {'conditions_met': short_conditions, 'vol_ratio': vol_ratio}
        ...
        if best_signal:
            ...
            try:
                ...
                report = (
                    f"🔴 *AUTO ENTERED SHORT (Strong Signal)*\n"
                    f"*Symbol:* {sym.replace('USDT','')}\n"
                    f"*Price:* {best_price:.4f}\n"
                    f"*Risk:* $0.5 | *Qty:* {qty_str}\n"
                    f"*SL:* {sl_price:.4f} | *TP:* {tp_price:.4f}\n"
...
async def check_telegram_updates(client, cmd_q, price_map):
    ...
    try:
        ...
        for update in updates:
            ...
            if text == '/help':
                help_text = (
                    "📋 **คำสั่ง TITAN PRO Bot** (v31.3 Enhanced)\n\n"
                    "💰 พิมพ์ชื่อเหรียญ เช่น `BTC`, `ETH`, `SOL` → วิเคราะห์แนวโน้ม 1D แบบละเอียด\n"
                    "📊 `/report` หรือ `/status` → สถานะบอทเต็มรูปแบบ\n"
                    "📈 `/pnl` → สรุปกำไร-ขาดทุนรวม + Win Rate\n"
                    "⭐ `/positions` → รายการ Position ที่เปิดอยู่ทั้งหมด\n"
                    "⏳ `/limits` → รายการ Limit Orders ที่รออยู่\n"
                    "🔍 `/analyze BTC` → วิเคราะห์ + กราฟ Fibonacci Retracement 4h (สวยงาม)\n"
                    "🛠 `/cancel` หรือ `/c` → ยกเลิก Limit Orders ทั้งหมด\n"
                    "🛑 `/closeall` หรือ `/a` → ปิดทุก Position ทันที\n"
                    "🚪 `/q` หรือ `/quit` → หยุดบอทอย่างปลอดภัย\n\n"
                    "🔄 `/spike on/off` → เปิด/ปิด Auto LONG เมื่อ Volume พุ่ง\n"
                    "🔄 `/shortsig on/off` → เปิด/ปิด Auto SHORT เมื่อสัญญาณลงแรง\n"
                    "📡 `/autostatus` → ดูสถานะ Auto Enter ทั้งหมด\n"
                    "_LFG!_ 🚀"
                )
                await send_telegram_report(help_text, chat_id)

            # ===================== ควบคุม Auto Enter ด่วน =====================
            elif text == '/spike on':
            ...
            elif text == '/shortsig off':
                global auto_short_signal_enabled
                auto_short_signal_enabled = False
                await send_telegram_report("🔴 *Strong Short Signal Auto SHORT* ปิดใช้งานแล้ว 🛑", chat_id)

            elif text == '/autostatus':
                status = (
                    f"📡 **สถานะระบบ Auto Enter ด่วน**\n\n"
                    f"Volume Spike (LONG): {'🟢 เปิด' if auto_spike_enabled else '🔴 ปิด'}\n"
                    f"Strong Short Signal: {'🟢 เปิด' if auto_short_signal_enabled else '🔴 ปิด'}"
                )
                await send_telegram_report(status, chat_id)
            ...
            # ===================== /pnl =====================
                ...
                total_pnl = sum(p['pnl'] for p in active)
                wins = sum(1 for _, y in brain.data if y == 1.0)
                total_trades = len(brain.data)
                win_rate = (wins / total_trades * 100) if total_trades > 0 else 0.0

                pnl_text = (
                    f"📈 **สรุปกำไร-ขาดทุน**\n\n"
                    f"💰 Total PNL: `{total_pnl:+,.2f}` USDT\n"
                    f"💳 Balance ปัจจุบัน: `{bal:,.2f}` USDT\n"
                )
                if total_trades > 0:
                    pnl_text += f"✅ Win Rate: `{win_rate:.1f}%` ({wins}/{total_trades} trades)\n"
                else:
                    pnl_text += f"⚠️ ยังไม่มีประวัติการเทรด\n"
                pnl_text += f"⭐ Position ที่เปิด: `{len(active)}/{MAX_OPEN_POSITIONS}`\n"
            ...
            # ===================== /analyze =====================
                ...
                # โค้ด /analyze ทั้งหมด (เหมือนเดิม - indent ถูกต้อง)
                try:
                    ...
                    ax.set_xlabel('แท่งเทียน', color='white', fontsize=14)
                    ax.set_ylabel('ราคา (USDT)', color='white', fontsize=14)
                    ax.tick_params(colors='white')
                    ax.grid(True, alpha=0.3, color='#424242')
                    ax.legend(facecolor='#121212', labelcolor='white', loc='upper left')
...
async def main():
    ...
    try:
        ...
        if telegram_bot:
            greeting = (
                ...
                "🔧 **การตั้งค่าปัจจุบัน (Optimized for $5 Capital)**\n"
                f"📊 Max Leverage: `{MAX_LEVERAGE}x`\n"
                f"⚠️ Risk per Trade: `{RISK_PER_TRADE_PERCENT * 100:.1f}%` ของพอร์ต\n"
                f"⭐ Max Open Positions: `{MAX_OPEN_POSITIONS}` ไม้พร้อมกัน\n"
                f"🟢 Long Threshold: `{SIGNAL_THRESHOLD_LONG}` คะแนน\n"
                f"🔴 Short Threshold: `{SIGNAL_THRESHOLD_SHORT}` คะแนน (เข้มงวดกว่า)\n"
                f"📈 ADX Threshold: `{ADX_THRESHOLD}` (เทรนด์ต้องแข็งแรง)\n"
                f"⏳ Pullback Target: `Dynamic 0-50%` (จาก AI) + Fibonacci\n"
                f"🛡️ SL Multiplier: `{ATR_SL_MULTIPLIER}x ATR`\n"
                f"🎯 TP Multiplier: `{ATR_TP_MULTIPLIER}x ATR` → RR 1:2.4\n"
                f"🛑 Min Balance to Trade: `{MIN_BALANCE_TO_TRADE}` USDT\n\n"
                "🔄 `/spike on/off` → เปิด/ปิด Auto LONG เมื่อ Volume พุ่ง\n"
                "🔄 `/shortsig on/off` → เปิด/ปิด Auto SHORT เมื่อสัญญาณลงแรง\n"
                "📡 `/autostatus` → ดูสถานะ Auto Enter ทั้งหมด\n"
                "🔥 สแกนจาก Top 100 Volume อัปเดตทุก 4 ชม.\n"
                "🛡️ ระบบป้องกันความเสี่ยงสูงสุดสำหรับทุนน้อย\n\n"
    ...
    while running:
        try:
            ...
            # --- ส่วนใหม่: ตั้ง SL/TP ให้ Position ที่ยังไม่มี ---
            for pos in active:
                sym = pos['symbol']
                side = pos['side']
                entry = pos['entry']
                ...
                # ถ้ายังไม่มี SL หรือ TP ให้ตั้งใหม่
                if pos['sl'] == 0.0 or pos['tp'] == 0.0:
                    print(f"{Fore.CYAN}→ พบ Position {sym} {side} ยังไม่มี SL/TP → ตั้งให้อัตโนมัติ")

                    try:
                        klines = await client.futures_klines(symbol=sym, interval="15m", limit=100)
                        df = calculate_indicators(klines)
                        atr_val = float(df.iloc[-1]['atr']) if not df.empty else entry * 0.015
                    except:
                        atr_val = entry * 0.015  # fallback
                    ...
                    if pos['sl'] == 0.0:
                        try:
                            ...
                            pos['sl'] = sl_price
                            print(f"{Fore.GREEN}ตั้ง SL สำเร็จ {sym} @ {sl_str}")
                        except Exception as e:
                            print(f"{Fore.RED}ตั้ง SL ล้มเหลว {sym}: {e}")

                    # ตั้ง TP ด้วย Algo Order
                    if pos['tp'] == 0.0:
                        try:
                            await client.futures_algo_new_order(
                                symbol=sym,
                            ...
                            pos['tp'] = tp_price
                            print(f"{Fore.GREEN}ตั้ง TP สำเร็จ {sym} @ {tp_str}")
                        except Exception as e:
            ...
            for sym in new_positions:
                ...
                try:
                    await client.futures_create_order(
                        symbol=sym,
                        side=SIDE_SELL if 'LONG' in report else SIDE_BUY,  # LONG ใช้ SELL, SHORT ใช้ BUY
                        type='STOP_MARKET',
                        stopPrice=f"{sl_price:.{p_prec}f}",
                        closePosition=True,
                        timeInForce='GTC',
                    ...
                    print(f"{Fore.RED}Failed to set SL for {sym}: {e}")

                # ตั้ง TP (TAKE_PROFIT_MARKET)
                try:
                    await client.futures_create_order(
                        symbol=sym,
                        side=SIDE_SELL if 'LONG' in report else SIDE_BUY,
                        type='TAKE_PROFIT_MARKET',
                        stopPrice=f"{tp_price:.{p_prec}f}",
                        closePosition=True,
                        timeInForce='GTC',
                    ...
                    print(f"{Fore.RED}Failed to set TP for {sym}: {e}")
            ...
            pending_symbols = set(o['symbol'] for o in pending_orders_detail)
                        # === PANIC SELL GUARD: ยกเลิก Limit ถ้าตลาด panic dump ===
            panic_symbols = set()

            for sym in list(pending_symbols):  # copy เพื่อป้องกัน modification
                try:
                    ...
                    if df.iloc[-1]['straight_down'] == 1:
                        panic_symbols.add(sym)

                except Exception as e:
                    print(f"{Fore.RED}Panic check error {sym}: {e}{Style.RESET_ALL}")

            if panic_symbols:
                print(f"{Fore.RED}{Style.BRIGHT}⚠️ PANIC SELL DETECTED! Cancelling limits: {', '.join(panic_symbols)}{Style.RESET_ALL}")
                
                await send_telegram_report(
                    f"⚠️ **PANIC SELL GUARD ACTIVATED**\n"
                    f"ตรวจพบการเทขายแบบ panic dump (ลงชัน ≥8% ใน 5 ชม.)\n"
                    f"ยกเลิก Pending Limit Orders อัตโนมัติ:\n" + 
                ...
                for order in open_orders:
                    if order['type'] == 'LIMIT' and order['symbol'] in panic_symbols:
                        try:
                            await client.futures_cancel_order(symbol=order['symbol'], orderId=order['orderId'])
                            print(f"{Fore.YELLOW}Cancelled panic limit: {order['symbol']} @ {order['price']}{Style.RESET_ALL}")
                            canceled_count += 1
                        except Exception as e:
                            print(f"{Fore.RED}Cancel failed {order['symbol']}: {e}{Style.RESET_ALL}")

                if canceled_count > 0:
                    # อัปเดต pending_orders_detail และ pending_symbols ใหม่
                    open_orders_all = await client.futures_get_open_orders()
                    pending_orders_detail = []
                    for o in open_orders_all:
                        if o['type'] == 'LIMIT':
            ...
            # === DETECT VOLUME SPIKE (โอกาส LONG ด่วน) ===
            ...
            # === DETECT STRONG SHORT SIGNALS (โอกาส SHORT ด่วน) ===
            ...
            if free_slots > 0 and bal >= MIN_BALANCE_TO_TRADE:
                ...
                if potential:
                    ...
                    for r in valid_signals:
                        ...
                        # เพิ่ม Volume Breakout Filter
                        if r['vol_breakout'] == 0: continue

                        # เพิ่ม RSI Confirmation
                        if r['side'] == 'LONG' and r['rsi'] > 30: continue  # Not oversold
                        if r['side'] == 'SHORT' and r['rsi'] < 70: continue  # Not overbought

                        # เพิ่ม Sentiment Filter
                        sentiment = await get_sentiment(r['symbol'])
                        if sentiment < 0.5: continue
                        ...
                        try:
                            ...
                            # ML Predicted Pullback
                            f = [r['rsi']/100, r['adx']/100, (r.get('macd', 0) - r.get('signal', 0)) / r['atr'], (current_p - r.get('ema200', current_p)) / r.get('ema200', current_p), r.get('vol_ratio', 1), r['score']/8.0, 1 if r['side'] == 'LONG' else 0]
                            pred_pullback = brain.get_pred_pullback(f)  # Dynamic 0-50%
                            ...
                            pred_pullback = brain.get_pred_pullback(f)  # Use dynamic even in fallback
                            if r['side'] == 'LONG':
                                limit_price_raw = current_p * (1 - (pred_pullback / 100))
                                side = SIDE_BUY
                            else:
                                limit_price_raw = current_p * (1 + (pred_pullback / 100))
                                side = SIDE_SELL
                        ...
                        try:
                            ...
                            await send_telegram_report(
                                ...
                                f"Pullback: {pred_pullback:.2f}% + Fib Support/Resistance\n"
                                f"Qty: {qty}")
