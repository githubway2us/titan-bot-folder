...
MAX_OPEN_POSITIONS = 6             # ลดจาก 50 → 3 ไม้พร้อมกัน (ป้องกัน over-exposure)
...
MIN_BALANCE_TO_TRADE = 10.0         # ลดจาก 100 → 4 (ให้บอทเริ่มเทรดได้แม้ทุนเหลือน้อย)

# ปรับการเข้าให้รอโซนดีจริง ๆ ตาม Fibonacci
ENTRY_PULLBACK_PERCENT = 35.8      # เพิ่มจาก 10.8 → 12.0% รอ pullback ลึกขึ้นเพื่อเข้าในจุดที่ดีกว่า (ตอนนี้ dynamic ด้วย ML)
...
class TitanBrain:
    ...
    def get_pred_pullback(self, f):
        ...
        return (conf / 100) * 50  # 0-50% based on confidence
    ...
    try:
        ...
        for update in updates:
            ...
            if text == '/help':
                ...
                try:
                    # ดึงข้อมูล 1D สำหรับวิเคราะห์แนวโน้มรายวัน
                    k = await client.futures_klines(symbol=sym, interval="1d", limit=500)
                    if not k or len(k) < 50:
                        await send_telegram_report(
                            f"💰 **{sym_input}**\n"
                            f"ราคา: {current_price:,.1f} USDT\n"
                            f"⚠️ ข้อมูลไม่เพียงพอสำหรับการวิเคราะห์ 1D",
                            chat_id
                        )
                        continue
                    ...
                    # สร้างข้อความวิเคราะห์แบบภาษาคน
                    report_text = (
                        f"📊 วิเคราะห์ {sym_input}/USDT (กรอบวัน – 1D)\n\n"

                        f"💰 ราคาตอนนี้: `{current_price:,.8f}` USDT\n"
                        f"📈📉 วันนี้: `{change_pct:+.8f}%`\n"
                        f"→ {'ราคาแทบไม่ขยับ ถือว่านิ่ง/พักตัว' if abs(change_pct) < 1 else 'ราคาขยับขึ้นชัด' if change_pct > 0 else 'ราคากดลงแรง'}\n\n"

                        f"🔍 ตัวชี้วัดทางเทคนิค (แปลให้ฟังง่าย)\n\n"

                        f"🔹 RSI (14): `{rsi_val:.1f}` → {'🟢 Oversold' if rsi_val < 30 else '🔴 Overbought' if rsi_val > 70 else '🟡 ปกติ'}\n"
                        f"→ {'อ่อนแอมาก มีโอกาสเด้ง' if rsi_val < 30 else 'ร้อนจัดแล้ว ระวังย่อ' if rsi_val > 70 else 'ยังไม่สุดโต่ง มีพื้นที่วิ่งได้ทั้งสองทาง'}\n"
                        f"→ {'เริ่มเอียงขึ้น' if rsi_val > 60 else 'เริ่มเอียงลง' if rsi_val < 40 else 'ยังกลาง ๆ'}\n\n"

                        f"🔹 ADX (14): `{adx_val:.1f}` → {'🟢 เทรนด์แข็งแรงมาก' if adx_val > 40 else '🟢 เทรนด์แข็งแรง' if adx_val > 30 else '🟡 เทรนด์อ่อน/ไซด์เวย์'}\n"
                        f"→ {'ตลาดมีเทรนด์ชัด ไม่นิ่ง' if adx_val > 30 else 'ตลาดยังนิ่ง ๆ หรือแกว่งในกรอบ'}\n"
                        f"→ ต้องดูทิศทางจาก EMA + MACD ร่วมด้วย\n\n"

                        f"🔹 MACD: {'🟢 Bullish' if macd_val > signal_val else '🔴 Bearish'}\n"
                        f"({macd_val:+.2f} vs Signal {signal_val:+.2f})\n"
                        f"→ {'โมเมนตัมฝั่งขึ้นกำลังมาแรง' if macd_val > signal_val else 'โมเมนตัมยังฝั่งลงอยู่'}\n"
                        f"→ {'มีแรงส่งเชิงบวก' if macd_val > signal_val else 'ยังไม่มีแรงซื้อหนุนชัด'}\n\n"

                        f"⚠️ โครงสร้างแนวโน้ม (จุดสำคัญที่สุด)\n\n"

                        f"🔹 EMA Alignment: {'🟢 ขาขึ้นแข็งแรงมาก (20>50>200)' if curr['ema20'] > curr['ema50'] > curr['ema200'] else '🔴 ขาลงแข็งแรงมาก (20<50<200)' if curr['ema20'] < curr['ema50'] < curr['ema200'] else '🟡 ไซด์เวย์ / ผสม'}\n\n"
                        f"EMA20 ≈ `{curr['ema20']:,.8f}`\n"
                        f"EMA50 ≈ `{curr['ema50']:,.8f}`\n"
                        f"EMA200 ≈ `{curr['ema200']:,.8f}`\n\n"
                        f"👉 {'โครงสร้างใหญ่ยังลงชัด การดีดคือแค่รีบาวด์' if curr['ema20'] < curr['ema50'] else 'โครงสร้างเริ่มกลับขึ้น มีโอกาสยืนยาว' if curr['ema20'] > curr['ema50'] > curr['ema200'] else 'ยังไม่ชัดเจน อยู่ในช่วงสะสม'}\n\n"

                        f"📦 สภาวะราคา & ความผันผวน\n\n"

                        f"🔹 Bollinger Bands: {'🟢 Breakout ขาขึ้นแรง' if current_price > curr['bb_upper'] else '🔴 Breakdown ขาลงแรง' if current_price < curr['bb_lower'] else '🟡 ในกรอบ'}\n"
                        f"บน ≈ `{curr['bb_upper']:,.8f}` | ล่าง ≈ `{curr['bb_lower']:,.8f}`\n"
                        f"→ {'ราคาอยู่บนกรอบ ใกล้เบรกขึ้น' if current_price > curr['ma20'] + (curr['bb_upper'] - curr['ma20']) * 0.7 else 'ราคาอยู่ล่างกรอบ ใกล้เด้ง' if current_price < curr['ma20'] - (curr['ma20'] - curr['bb_lower']) * 0.7 else 'ราคาอยู่กลางกรอบ ยังสะสมพลัง'}\n\n"

                        f"🔹 Volume: {'🔥 สูงมาก' if vol_ratio > 2.0 else '🟢 สูง' if vol_ratio > 1.5 else 'ปกติ'}\n"
                        f"→ {'มีแรงซื้อ/ขายเข้ามาแล้ว น่าเชื่อถือ' if vol_ratio > 1.5 else 'ยังเบา ตลาดยังไม่ตัดสินใจ'}\n\n"

                        f"🔹 ATR (14): ~`{curr['atr']:,.0f}`\n"
                        f"→ ความผันผวนวันละ ±{curr['atr']:,.0f} USDT\n"
                        f"→ ตั้ง SL/TP ตามนี้ เช่น 1.5–2x ATR\n\n"

                        f"🧠 สรุปภาพรวม \n\n"
                        f"{'🔴 โครงสร้างใหญ่ยังลงชัด – ระวังรีบาวด์จบแล้วลงต่อ' if curr['ema20'] < curr['ema50'] else '🟢 โครงสร้างเริ่มกลับขึ้น'}\n"
                        f"{'🟢 โมเมนตัมสั้นเริ่มบวก' if macd_val > signal_val else '🔴 โมเมนตัมยังลบ'}\n"
                        f"🟡 ราคา{' Sideway ในกรอบ' if current_price < curr['bb_upper'] and current_price > curr['bb_lower'] else ' กำลังเบรก'}\n"
                        f"📊 Volume {'ต่ำ → ยังไม่ยืนยัน' if vol_ratio < 1.5 else 'มาแล้ว → เชื่อได้'}\n\n"

                        f"❗ สรุปสั้น: {'ยังไม่ใช่จังหวะกลับเทรนด์จริง' if curr['ema20'] < curr['ema50'] else 'เริ่มมีโอกาสกลับตัว'}\n\n"

                        f"📌 มุมมองกลยุทธ์\n\n"
                        f"• Long → {'เหมาะเล่นสั้น/เด้ง/Scalp เท่านั้น' if curr['ema20'] < curr['ema50'] else 'เริ่มน่าเล่น Swing ได้'}\n"
                        f"• Short → {'ได้เปรียบโครงสร้าง ถ้าราคาไม่ผ่าน EMA50' if curr['ema20'] < curr['ema50'] else 'ระวัง Short Squeeze'}\n"
                        f"• ถือยาว → {'รอราคายืนเหนือ EMA50 + Volume แรงก่อน' if curr['ema20'] < curr['ema200'] else 'เริ่มพิจารณาได้'}\n\n"
                    ...
                    print(f"{Fore.RED}Error analyzing {sym} (1D) for Telegram: {e}")
                    await send_telegram_report(
                        f"💰 **{sym_input}**\n"
                        f"ราคา: {current_price:,.1f} USDT\n"
                        f"⚠️ ไม่สามารถวิเคราะห์ 1D ได้ในขณะนี้",
                        chat_id
                    )
